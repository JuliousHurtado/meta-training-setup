...
class HatNet(torch.nn.Module):
    ...
    def print_model_size(self):
        count_all = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print('Architecture size: %s parameters (%sB)' % (self.pretty_print(count_all),
                                                                    self.pretty_print(4*(count_all))))
